...
Shader::Shader(const std::string& name, const std::string& filename, ShaderType type)
: name(name), type(type)
{
    switch (type)
    {
...
Shader::load(const std::string& filename)
{
    ...
    std::string source = std::string((std::istreambuf_iterator<char>(shaderSourceFileHandle)),
