...
// for std::map
static inline bool
operator<(const vertex_index& a, const vertex_index& b)
{
    if (a.v_idx != b.v_idx)
...
static inline void
parseFloat2(float& x, float& y, const char*& token)
{
    x = parseFloat(token);
    y = parseFloat(token);
}
...
static inline void
parseFloat3(float& x, float& y, float& z, const char*& token)
{
    x = parseFloat(token);
    y = parseFloat(token);
    z = parseFloat(token);
...
std::string
LoadMtl(std::map<std::string, int>& material_map,
...
std::string
MaterialFileReader::operator()(const std::string&          matId,
                               std::vector<material_t>&    materials,
...
std::string
LoadObj(std::vector<shape_t>&    shapes,
        ...
        MaterialReader&          readMatFn)
{
    ...
    while (inStream.peek() != -1)
    {
        ...
        if ((0 == strncmp(token, "usemtl", 6)) && isSpace((token[6])))
        {
            ...
            // bool ret = exportFaceGroupToShape(shape, vertexCache, v, vn, vt, faceGroup, material,
            // name, false);
        ...
        if (token[0] == 'g' && isSpace((token[1])))
        {
            ...
            bool ret = exportFaceGroupToShape(
                shape, vertexCache, v, vn, vt, faceGroup, material, name, true);
            if (ret)
            {
