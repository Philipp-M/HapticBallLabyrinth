...
void
Scene::loadObjFile(std::string filename, std::string& materialRefFolder)
{
    ...
    for (auto& m : objectMaterial)
    {
        ...
        {
            std::cout << m.name << " diffuse: " << m.diffuse[0] << "/" << m.diffuse[1] << "/"
            ...
            MaterialManager::getInstance().addMaterial(std::make_shared<Material>(
                ...
                Material::Color(m.specular[0], m.specular[1], m.specular[2]),
                m.shininess));
        }
    }
    ...
    for (auto& object : objectShape)
    {
        ...
        {
            models.push_back(std::make_shared<GraphicsModel>(
                object.mesh,
                object.name,
...
Scene::Scene(std::vector<std::string>& objFilePaths,
             std::string&              materialRefFolder,
             int                       width,
             int                       height)
: camera(height,
         width,
         ...
         -90.0)
{
    ...
    lights.push_back(std::make_shared<PointLight>(
        glm::vec3(10.0, 60.0, 10.0), Material::Color(glm::vec3(1.0, 1.0, 1.0)), 150, 0.3, 0.2));
    lights.push_back(std::make_shared<PointLight>(
        glm::vec3(-10.0, 60.0, -10.0), Material::Color(glm::vec3(1.0, 1.0, 1.0)), 150, 0.3, 0.2));
    lights.push_back(std::make_shared<PointLight>(
        glm::vec3(10.0, 60.0, -10.0), Material::Color(glm::vec3(1.0, 1.0, 1.0)), 150, 0.3, 0.2));
    lights.push_back(std::make_shared<PointLight>(
        glm::vec3(-10.0, 60.0, 10.0), Material::Color(glm::vec3(1.0, 1.0, 1.0)), 150, 0.3, 0.2));
    lights.push_back(std::make_shared<PointLight>(
        glm::vec3(0.0, 60.0, 0.0), Material::Color(glm::vec3(1.0, 1.0, 1.0)), 1500, 0.3, 0.2));
}

void
Scene::resetSceneWithNewLabyrinth(std::string& objFilePaths, std::string& materialRefFolder)
{
...
Scene::draw(ShaderProgram& shaderProgram)
{
    ...
    for (auto& model : models)
    {
        shaderProgram.setMatrixUniform4f("MVPMatrix",
...
void
Scene::rotateModelAroundAxis(int id, int axis, float angle)
{
...
void
Scene::resetModelRotationAroundAxis(int id)
{
    models[id]->resetRotationMatrixAxis();
}

std::shared_ptr<GraphicsModel>
Scene::getModelByName(std::string name)
{
    for (auto& model : models)
    {
...
void
Scene::setWindwoSize(int width, int height)
{
