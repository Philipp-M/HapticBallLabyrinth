...
void
PointLight::addToShader(ShaderProgram& shaderProgram, GLuint num)
{
    shaderProgram.setUniform3f("allPointLights[" + SSTR(num) + "].position", position);
    shaderProgram.setUniform3f("allPointLights[" + SSTR(num) + "].colorIntensity",
                               intensity * color.rgb);
